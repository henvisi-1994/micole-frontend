...
export class AcheivementResolver implements Resolve<Acheivement[]> {

  constructor(private courseService: CourseService) {}

  resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Acheivement[] | Observable<Acheivement[]> | Promise<Acheivement[]> {
    const subjectId = route.paramMap.get('subject_id');
    const courseId = route.parent.parent.parent.paramMap.get('id');
    return this.courseService.getAcheivements(courseId, subjectId)
  }
